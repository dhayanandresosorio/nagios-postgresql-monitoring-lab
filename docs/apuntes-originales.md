...
Para llevar a cabo esta práctica lo primero que debemos hacer es preparar el escenario. Para esta práctica se usaron dos máquinas principales: una máquina servidor con PostgreSQL y una máquina cliente con Nagios.

- M.Server → IP: 192.168.14.50
- M.Client → IP: 192.168.14.27

En el entorno original se trabajó con máquinas virtuales conectadas por red. Para esta versión del repositorio me centro en el servidor PostgreSQL y en el cliente Nagios.

Ambas máquinas deben estar dentro de la misma red para que Nagios pueda comprobar el servicio PostgreSQL de forma remota.
...
```bash
lab_user@nagioserver:~$ sudo apt update
...
```bash
lab_user@nagioserver:~$ sudo apt install -y postgresql postgresql-client
...
```bash
lab_user@nagioserver:~$ systemctl status postgresql
...
```bash
lab_user@nagioserver:~$ sudo nano /etc/postgresql/*/main/postgresql.conf
``
...
```bash
lab_user@nagioserver:~$ sudo nano /etc/postgresql/*/main/pg_hba.conf
```
...
```bash
lab_user@nagioserver:~$ sudo systemctl restart postgresql
```

Y comprobar que tras reiniciarse, todo sigue en funcionamiento.

```bash
lab_user@nagioserver:~$ sudo systemctl status postgresql
● postgresql.service - PostgreSQL RDBMS
     Loaded: loaded (/usr/lib/systemd/system/postgresql.service; enabled; prese>
...
```bash
lab_user@nagioserver:~$ sudo -u postgres psql
psql (16.11 (Ubuntu 16.11-0ubuntu0.24.04.1))
Type "help" for help.

postgres=#
```
Creamos el usuario **nagios_monitor** con la contraseña de ejemplo.

```bash
postgres=# CREATE ROLE nagios_monitor WITH LOGIN PASSWORD 'CHANGE_ME_DB_PASSWORD';
CREATE ROLE
```
...
```bash
postgres=# CREATE DATABASE nagiosdatabase OWNER nagios_monitor;
CREATE DATABASE
postgres=# \q
```
...
```bash
lab_user@nagioserver:~$ ss -lntp | grep 5432
...
```bash
lab_user@nagios:~$ sudo apt update
```
...
```bash
lab_user@nagios:~$ sudo apt install -y nagios4 apache2 php libapache2-mod-php nagios-plugins
```
...
```bash
lab_user@nagios:~$ sudo a2enmod cgi

lab_user@nagios:~$ sudo systemctl restart apache2
```
...
```bash
lab_user@nagios:~$ sudo systemctl enable --now nagios4
```
...
```bash
lab_user@nagios:~$ sudo htpasswd -c /etc/nagios4/htpasswd.users nagiosadmin
```
...
```bash
lab_user@nagios:~$ systemctl status nagios4 --no-pager
```

<img src="../img/systemctl-status-nagios4.png">

Si sale activo y sin errores, sé que el servicio está corriendo correctamente, por tanto todo va bien
...
```bash
lab_user@nagios:~$ sudo nagios4 -v /etc/nagios4/nagios.cfg
```

<img src="../img/sudo-nagios4.png">
...
```bash
lab_user@nagios:~$ sudo chown nagios:nagios /var/lib/nagios/.pgpass
lab_user@nagios:~$ sudo chmod 600 /var/lib/nagios/.pgpass
```
Lo siguiente será indicarle a Nagios, como usar el plugin check_pgsql, por tanto lo definiremos dentro del archivo commands.cfg
Entramos a:
```bash
...
define command {
    ...
    command_line    /usr/lib/nagios/plugins/check_pgsql -H $HOSTADDRESS$ -p 5432 -U nagios_monitor -d nagiosdatabase
}
```
...
```bash
lab_user@nagios:~$ sudo systemctl restart nagios4
```
...
lab_user@nagios:~$ sudo apt install -y perl libdbi-perl libdbd-pg-perl postgresql-client wget
```
...
```bash
lab_user@nagios:~$ cd /tmp
```

Lo descargamos con:

```bash
lab_user@nagios:/tmp$ wget -O check_postgres.pl https://raw.githubusercontent.com/bucardo/check_postgres/master/check_postgres.pl
...
```bash
lab_user@nagios:/tmp$ sudo install -m 0755 check_postgres.pl /usr/lib/nagios/plugins/check_postgres.pl
```
...
```bash
lab_user@nagios:/tmp$ cd /usr/lib/nagios/plugins
lab_user@nagios:/usr/lib/nagios/plugins$ sudo mkdir postgres
lab_user@nagios:/usr/lib/nagios/plugins$ cd postgres

lab_user@nagios:/usr/lib/nagios/plugins/postgres$  sudo perl ../check_postgres.pl --symlinks
```
...
```bash
lab_user@nagios:/usr/lib/nagios/plugins/postgres$ cd ..
lab_user@nagios:~$ sudo -u nagios nano /var/lib/nagios/.pgpass
```
...
```bash
lab_user@nagios:~$ sudo -u nagios /usr/lib/nagios/plugins/check_postgres.pl \
--action=backends --host=192.168.14.50 \
--db=nagiosdatabase --dbuser=nagios_monitor
```
...
```bash
lab_user@nagios:~$ sudo nano /etc/nagios4/commands.cfg
```
...
command_line /usr/lib/nagios/plugins/check_postgres.pl --action=backends --host=$HOSTADDRESS$ --db=nagiosdatabase --dbuser=nagios_monitor --warning=5 --critical=10
}
```
...
Crearemos nuestro propio archivo para separar esta configuración del resto de archivos de Nagios.

```bash
lab_user@nagios:~$ sudo nano /etc/nagios4/conf.d/postgresql-advanced.cfg
```
...
define service{
    ...
    check_command           check_postgres_backends
}
```
...
```bash
lab_user@nagios:~$ psql "host=192.168.14.50 dbname=nagiosdatabase user=nagios_monitor"
```
